...
{
    public class RPCCubeController : MonoBehaviourPunCallbacks//, IPunObservable
    {
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            {
              photonView.RPC("ChangeColour", RpcTarget.AllBufferedViaServer, cubeMaterialIndex+1);
              //cubeMaterialIndex++;
              //gameObject.GetComponent<MeshRenderer>().material = cubeMaterials[cubeMaterialIndex];
            }
            else
            {
                photonView.RPC("ChangeColour", RpcTarget.AllBufferedViaServer, 0);
                //cubeMaterialIndex = 0;
                //gameObject.GetComponent<MeshRenderer>().material = cubeMaterials[cubeMaterialIndex];
            }
        }
